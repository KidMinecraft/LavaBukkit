...
public class NetLoginHandler extends NetHandler
{
    ...
    private String loginServerId = Long.toString(rand.nextLong(), 16); // CraftBukkit - Security fix, was ""
    ...
    public void handleClientCommand(Packet205ClientCommand par1Packet205ClientCommand)
    {
        if (par1Packet205ClientCommand.forceRespawn == 0)
        {
            ...
            {
                (new ThreadLoginVerifier(this, mcServer.server)).start(); // CraftBukkit - add server
            }
            else
            {
    ...
    {
    	// CraftBukkit start
        EntityPlayerMP s = this.mcServer.getConfigurationManager().allowUserToConnect(this, this.clientUsername, this.hostname);
        ...
        {
            EntityPlayerMP var2 = this.mcServer.getConfigurationManager().createPlayerForUser(s); // CraftBukkit
    ...
    {
    	if (this.myTCPConnection.getSocket() == null) return; // CraftBukkit - fix NPE when a client queries a server that is unable to handle it.
        ...
        try
        {
            ...
            // CraftBukkit
            org.bukkit.event.server.ServerListPingEvent pingEvent = org.bukkit.craftbukkit.event.CraftEventFactory.callServerListPingEvent(this.mcServer.server, getSocket().getInetAddress(), this.mcServer.getMOTD(), var2.getCurrentPlayerCount(), var2.getMaxPlayers());
                
            if (par1Packet254ServerPing.field_82559_a == 1)
            {
            	// CraftBukkit
            	List var4 = Arrays.asList(new Serializable[] {Integer.valueOf(1), Integer.valueOf(51), this.mcServer.getMinecraftVersion(), pingEvent.getMotd(), Integer.valueOf(pingEvent.getNumPlayers()), Integer.valueOf(pingEvent.getMaxPlayers())});
                Object var6;
            ...
            {
            	// CraftBukkit
                var3 = pingEvent.getMotd() + "\u00a7" + pingEvent.getNumPlayers() + "\u00a7" + pingEvent.getMaxPlayers();
            }
