...
public final class CraftItemStack extends ItemStack {
    ...
    public int removeEnchantment(Enchantment ench) {
        ...
        for (int i = 0; i < size; i++) {
            NBTTagCompound enchantment = (NBTTagCompound) list.tagAt(i);
    ...
    static Map<Enchantment, Integer> getEnchantments(net.minecraft.item.ItemStack item) {
        ...
        for (int i = 0; i < list.tagCount(); i++) {
            int id = 0xffff & ((NBTTagCompound) list.tagAt(i)).getShort(ENCHANTMENTS_ID.NBT);
            int level = 0xffff & ((NBTTagCompound) list.tagAt(i)).getShort(ENCHANTMENTS_LVL.NBT);
